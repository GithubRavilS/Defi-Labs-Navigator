...
function sanitizeSheetCellForDisplay(val, fallback = "") {
  if (val == null || val === "") return fallback;
  const s = String(val).trim();
  if (!s || isSheetErrorCellValue(s)) return fallback;
  return s;
}
...
/** ETH / BTC / Stablecoin — один лист ethereum, колонка Q «Битва». */
function isEthereumMultiCategorySheet(title) {
  if (isEthPoolBattleSheet(title)) return true;
  return normalizeSheetTitleKey(title) === "ethereum";
}

/** Листы bitcoin и stables больше не читаем. */
function isDeprecatedPoolBattleSheet(title) {
  const n = normalizeSheetTitleKey(title);
  return n === "bitcoin" || n === "stables";
}

/**
 * Служебные дампы (Revert/Krystal exports, Meta, raw dumps) — не категории навигатора.
 * Их чтение (десятки–сотни тысяч строк) валит /api/sheet-data stack overflow / timeout.
 */
function isNavigatorExportOrMetaSheet(title) {
  const n = normalizeSheetTitleKey(title);
  if (!n) return true;
  if (/\bmeta\b/.test(n)) return true;
  if (/^krystal\b/.test(n) || /\bkrystal\b/.test(n)) return true;
  if (/^revert\b/.test(n) || /\brevert\b/.test(n)) return true;
  if (/raw[_\s-]?dump/.test(n) || /stable_raw/.test(n)) return true;
  if (/\bpartial\b/.test(n) && /\d{4}-\d{2}-\d{2}/.test(n)) return true;
  if (/\bfull\b/.test(n) && /\d{4}-\d{2}-\d{2}/.test(n)) return true;
  // Timestamped dump tabs: "... 2026-08-07 1658"
  if (/\d{4}-\d{2}-\d{2}\s+\d{3,4}\b/.test(n)) return true;
  return false;
}

/** Только листы, которые реально кормят UI-категории. */
function shouldReadSheetAsNavigatorCategory(title, categoryIdSet) {
  if (isDeprecatedPoolBattleSheet(title)) return false;
  if (isNavigatorExportOrMetaSheet(title)) return false;
  if (isRwaPoolBattleSheet(title) || isEthereumMultiCategorySheet(title)) return true;
  const fromSheetName = sheetNameToCategoryId(title);
  if (fromSheetName && categoryIdSet.has(fromSheetName)) return true;
  // Известные вкладки навигатора вне metadata-id (lending и т.п. уже в set).
  const allow = new Set([
    "ethereum",
    "bitcoin",
    "stables",
  ...
  if (fromSheetName && allow.has(fromSheetName)) return true;
  return false;
}

function appendToolsSafe(target, tools) {
  if (!tools || !tools.length) return;
  // push(...huge) → Maximum call stack size exceeded
  const CHUNK = 5000;
  for (let i = 0; i < tools.length; i += CHUNK) {
    const part = tools.slice(i, i + CHUNK);
    for (let j = 0; j < part.length; j++) target.push(part[j]);
  }
}

function isPoolBattleLiquidityCategory(categoryId) {
  return categoryId === "ethereum" || categoryId === "bitcoin" || categoryId === "stables";
}

/** Колонка Q «Битва»: Ethereum / Bitcoin / Stablecoin. */
function categoryIdFromBattleColumn(rawVal, displayVal) {
  const disp =
    displayVal != null && String(displayVal).trim() !== "" ? String(displayVal).trim() : "";
  const s = (disp || String(rawVal == null ? "" : rawVal).trim()).toLowerCase().replace(/ё/g, "е");
  if (!s) return "ethereum";
  if (s === "ethereum" || /эфир/.test(s)) return "ethereum";
  if (s === "bitcoin" || /биткоин|\bbtc\b/.test(s)) return "bitcoin";
  if (s === "stablecoin" || /стейбл|stablecoin|\bstables?\b/.test(s)) return "stables";
  return "";
}

function resolveBattleColumnIndex(headers) {
  return findHeaderColumn(headers, 0, [/^битва$|битва.*атриб|battle.*attr/i], 16);
}
...
function mapUnifiedToolColumns(headers) {
  return {
    ...
    battleCol: findHeaderColumn(headers, 0, [/^битва$|битва.*атриб|battle.*attr/i], 16),
  };
}
...
function enrichPoolBattleFields(name, desc, platform, chain, fee) {
  ...
  return {
    platform: platform || "",
    chain: normalizeChainDisplayLabel(chain),
    fee: fee || "",
  };
}

/** Revert пишет mainnet — на сайте это Ethereum; иначе фильтр «Ethereum» прячет пул. */
function normalizeChainDisplayLabel(chain) {
  const c = String(chain || "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "");
  if (!c) return "";
  if (c === "ethereum" || c === "mainnet" || c === "eth") return "Ethereum";
  if (c === "arbitrum" || c === "arbitrumone" || c === "arb") return "Arbitrum";
  if (c === "optimism" || c === "opmainnet" || c === "op") return "Optimism";
  if (c === "base") return "Base";
  if (c === "polygon" || c === "matic") return "Polygon";
  if (c === "bsc" || c === "bnb" || c === "binancesmartchain" || c === "bnbsmartchain")
    return "BNB";
...
function parseToolsFromSheet(rows, displayRows, categoryId, startToolId, options) {
  const opts = options || {};
  const multiCategory = opts.multiCategory === true;
  const tools = [];
  ...
  const apyCol = unified
    ? resolveUnifiedApyColumnIndex(headers, multiCategory ? "ethereum" : categoryId)
  ...
  const battleCol =
    unified && multiCategory
      ? umap.battleCol >= 0
        ? umap.battleCol
        : resolveBattleColumnIndex(headers)
      : -1;
  const liquidityStatusCol =
    unified && (multiCategory || isPoolBattleLiquidityCategory(categoryId))
      ? umap.liquidityStatusCol >= 0
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    if (multiCategory) {
      rowCategoryId = categoryIdFromBattleColumn(
        battleCol >= 0 ? rw[battleCol] : "",
        battleCol >= 0 ? pickDisplay(r, battleCol, rw) : "",
      );
      if (!rowCategoryId) continue;
    }

    if (
      isPoolBattleLiquidityCategory(rowCategoryId) &&
      liquidityStatusCol >= 0 &&
      !isEthereumLiquidityStatusOpen(rw[liquidityStatusCol], pickDisplay(r, liquidityStatusCol, rw))
    ) {
      continue;
    }

    let name = sanitizeSheetCellForDisplay(pickDisplay(r, nameCol, rw) || pick(rw, nameCol));
    if (!name && unified && platformCol >= 0) {
      name = sanitizeSheetCellForDisplay(pickPlatformValue(r, platformCol, rw, displayRows));
    }
    if (!name) name = "—";

    const apyDisp = apyCol >= 0 && displayRows[r] ? displayRows[r][apyCol] : "";
    const apyRaw = apyCol >= 0 ? rw[apyCol] : "";
    let apy = formatApyForApi(apyRaw, apyDisp);
    if (isSheetErrorCellValue(apy)) apy = "0";
    ...
    let link = sanitizeSheetCellForDisplay(pickDisplay(r, linkCol, rw) || pick(rw, linkCol), "#");
    let desc = descCol >= 0 ? sanitizeSheetCellForDisplay(pick(rw, descCol)) : "";
    const pair = sanitizeSheetCellForDisplay(
      pairCol >= 0 ? pickDisplay(r, pairCol, rw) || pick(rw, pairCol, "") : "",
    );
    let platform =
      platformCol >= 0
        ? sanitizeSheetCellForDisplay(pickPlatformValue(r, platformCol, rw, displayRows))
        : "";
    if (!platform) platform = name === "—" ? "—" : name;
    if (shouldSkipToolRow(rowCategoryId, pair, name, platform)) continue;
    ...
    let chain = sanitizeSheetCellForDisplay(
      chainCol >= 0 ? pick(rw, chainCol) || pickDisplay(r, chainCol, rw) : "",
    );
...
async function getNavigatorDataFromSheets() {
  ...
  for (let s = 1; s < sheetList.length; s++) {
    ...
    if (isDeprecatedPoolBattleSheet(title)) continue;
    if (!shouldReadSheetAsNavigatorCategory(title, categoryIdSet)) continue;

    let categoryId = "";
    let multiCategory = false;
    if (isRwaPoolBattleSheet(title)) categoryId = "rwa";
    else if (isEthereumMultiCategorySheet(title)) {
      multiCategory = true;
    } else {
      const fromSheetName = sheetNameToCategoryId(title);
      ...
      else if (fromOrder && categoryIdSet.has(fromOrder)) categoryId = fromOrder;
      else continue;
    }
    ...
    // Hard cap: dump tabs should already be skipped; still guard runaway sheets.
    if (rows.length > 5000) {
      console.warn(`[sheet-get-data] skip oversized sheet "${title}" rows=${rows.length}`);
      continue;
    }

    const parsed = multiCategory
      ? parseToolsFromSheet(rows, displayRows, null, toolId, { multiCategory: true })
      : parseToolsFromSheet(rows, displayRows, categoryId, toolId);
    appendToolsSafe(allTools, parsed.tools);
