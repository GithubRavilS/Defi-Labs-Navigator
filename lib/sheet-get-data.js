...
function formatFeeForApi(rawVal, displayVal) {
  const rawN = feeToPercentNumber(rawVal);
  if (Number.isFinite(rawN)) return String(rawN);
  const disp = displayVal != null ? String(displayVal).trim() : "";
  const dispN = feeToPercentNumber(disp);
  if (Number.isFinite(dispN)) return String(dispN);
...
function pickRangeValue(r, idx, rw) {
  if (idx < 0) return "";
  const raw = rw[idx];
  if (typeof raw === "number" && Number.isFinite(raw)) {
    if (raw > 50000 || raw < 0) return "";
    return formatSheetNumber(raw, 4);
  }
  return sanitizeRangeCell(String(raw == null ? "" : raw).trim());
}

function formatOpenDateForApi(rawVal) {
  ...
  const s = String(rawVal == null ? "" : rawVal).trim();
  if (!s || isDateLikeLabel(s)) return "";
...
function pickPlatformValue(r, idx, rw, displayRows) {
  if (idx < 0) return "";
  const raw =
    rw[idx] !== undefined && rw[idx] !== null && rw[idx] !== ""
      ? String(rw[idx]).trim()
      : "";
...
function parseServiceAccountJson(raw) {
  ...
  if (!s) return null;
  s = s.replace(/^GOOGLE_SERVICE_ACCOUNT_JSON\s*=\s*/i, "");
...
function parseToolsFromSheet(rows, displayRows, categoryId, startToolId) {
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    let openDate = "";
    if (openDateCol >= 0) {
      openDate = formatOpenDateForApi(rw[openDateCol]);
    }
    ...
    let platform =
      platformCol >= 0 ? pickPlatformValue(r, platformCol, rw, displayRows) : "";
    ...
    let priceMin =
      minPriceCol >= 0 ? pickRangeValue(r, minPriceCol, rw) : "";
    let priceMax =
      maxPriceCol >= 0 ? pickRangeValue(r, maxPriceCol, rw) : "";
    if (categoryId === "rwa") {
