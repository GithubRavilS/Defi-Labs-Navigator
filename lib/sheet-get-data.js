...
function normalizePairKey(pair) {
  return String(pair || '').toLowerCase().replace(/\s+/g, '').replace(/[·•]/g, '/');
}
...
function isRwaPoolBattleSheet(title) {
  const n = String(title || '').trim().toLowerCase().replace(/\s+/g, ' ').replace(/ё/g, 'е');
...
function pickRangeValue(r, idx, rw, displayRows) {
  ...
  if (disp !== undefined && disp !== null && disp !== '') return sanitizeRangeCell(String(disp).trim());
...
function findCol(row, names) {
  for (const name of names) {
    ...
    for (let i = 0; i < row.length; i++) {
      if (String(row[i] || '').toLowerCase().trim() === n) return i;
    }
  }
  return -1;
}
...
function parseToolsFromSheet(rows, displayRows, categoryId, startToolId) {
  ...
  const apyCol = unified ? (umap.apyCol >= 0 ? umap.apyCol : findApyColumnIndex(headers)) : findApyColumnIndex(headers);
  ...
  const platformCol = unified ? umap.platformCol : colIndex(['platform', 'платформа', 'dex', 'protocol', 'протокол']);
  const feeCol = unified ? umap.feeCol : colIndex(['fee', 'fee tier', 'fee_tier', 'fitier', 'fi tier', 'комиссия', 'tier', 'уровень']);
  const chainCol = unified ? umap.chainCol : colIndex(['chain', 'blockchain', 'network', 'сеть', 'блокчейн']);
  const typeCol = colIndex(['type', 'тип', 'position type', 'pool type', 'тип позиции']);
  const minPriceCol = unified ? umap.minPriceCol : findHeaderColumn(headers, 0, [/мин.*диапаз|min.*range/], -1);
  const maxPriceCol = unified ? umap.maxPriceCol : findHeaderColumn(headers, 0, [/макс.*диапаз|max.*range/], -1);
  const openDateCol = unified ? umap.openDateCol : -1;

  const pick = (row, idx, def = '') =>
    idx >= 0 && row[idx] !== undefined && row[idx] !== null && row[idx] !== '' ? String(row[idx]).trim() : def;
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    if ((!period || unified) && openDateCol >= 0) {
      period = periodFromOpenDateCell(rw[openDateCol], pickDisplay(r, openDateCol, rw) || pick(rw, openDateCol));
    }
    if (!period) period = '7d';
    ...
    let priceMin = unified ? pickRangeValue(r, minPriceCol, rw, displayRows) : pickDisplay(r, minPriceCol, rw) || pick(rw, minPriceCol);
    let priceMax = unified ? pickRangeValue(r, maxPriceCol, rw, displayRows) : pickDisplay(r, maxPriceCol, rw) || pick(rw, maxPriceCol);
    if (categoryId === 'rwa') {
...
async function getNavigatorDataFromSheets() {
  ...
  if (!sheetList.length) return { categories: [], tools: [], apiVersion: API_VERSION, source: 'sheets-direct' };
  ...
  const descI = findCol(catHeaders, ['description']) >= 0 ? findCol(catHeaders, ['description']) : 3;
  ...
  for (let s = 1; s < sheetList.length; s++) {
    ...
    try {
      const [rawRes, fmtRes] = await Promise.all([
        sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: q, valueRenderOption: 'UNFORMATTED_VALUE' }),
        sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: q, valueRenderOption: 'FORMATTED_VALUE' }),
      ]);
