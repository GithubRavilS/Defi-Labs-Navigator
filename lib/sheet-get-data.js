...
function pickRangeValue(r, idx, rw) {
  ...
  if (Number.isFinite(n)) {
    if (n > RANGE_PRICE_CEILING || n < 0) return "";
...
function parseToolsFromSheet(rows, displayRows, categoryId, startToolId) {
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    let priceMin = minPriceCol >= 0 ? pickRangeValue(r, minPriceCol, rw) : "";
    let priceMax = maxPriceCol >= 0 ? pickRangeValue(r, maxPriceCol, rw) : "";
    if (categoryId === "rwa") {
