...
/** Ethereum col P: скрываем только явно closed/false/#error; пусто = открыта (Krystal). */
...
function isKrystalPositionLink(link) {
  return /cloud-ui\.krystal\.app\/positions\//i.test(String(link || ""));
}
...
function poolBattleRowQualityScore(tool) {
  let score = 0;
  if (!isKrystalPositionLink(tool.link)) score += 20;
  const feeNum = feeToPercentNumber(tool.fee);
  if (Number.isFinite(feeNum) && feeNum > 0) score += 10;
  if (tool.priceMin && tool.priceMax) score += 5;
  if (tool.openDate) score += 3;
  return score;
}

function isFeeTierMissingOrZero(tool) {
  const n = feeToPercentNumber(tool.fee);
  return !Number.isFinite(n) || n === 0;
}

function enrichPoolBattleToolsFromRevertByPositionId(tools) {
  if (!tools || !tools.length) return [];
  const revertByPos = {};
  for (const t of tools) {
    if (isKrystalPositionLink(t.link)) continue;
    const posId = extractPositionIdFromLink(t.link);
    if (!posId) continue;
    if (
      !revertByPos[posId] ||
      poolBattleRowQualityScore(t) > poolBattleRowQualityScore(revertByPos[posId])
    )
      revertByPos[posId] = t;
  }
  return tools.map((tool) => {
    if (!isKrystalPositionLink(tool.link) || !isFeeTierMissingOrZero(tool)) return tool;
    const posId = extractPositionIdFromLink(tool.link);
    const rev = posId ? revertByPos[posId] : null;
    if (!rev || isFeeTierMissingOrZero(rev)) return tool;
    const copy = { ...tool };
    copy.fee = normalizeFeeValue(rev.fee);
    if (!copy.priceMin && rev.priceMin) copy.priceMin = rev.priceMin;
    if (!copy.priceMax && rev.priceMax) copy.priceMax = rev.priceMax;
    if (!copy.openDate && rev.openDate) copy.openDate = rev.openDate;
    if (!copy.period) copy.period = rev.period;
    return copy;
  });
}

function dedupePoolBattleByPositionId(tools) {
  if (!tools || !tools.length) return [];
  const byPos = {};
  const noPos = [];
  for (const tool of tools) {
    ...
    const existing = byPos[posId];
    if (!existing || poolBattleRowQualityScore(tool) > poolBattleRowQualityScore(existing))
      byPos[posId] = tool;
  }
  return noPos.concat(Object.values(byPos));
}
...
async function getNavigatorDataFromSheets() {
  ...
  return {
    categories,
    tools: dedupePoolBattleByPositionId(enrichPoolBattleToolsFromRevertByPositionId(allTools)),
