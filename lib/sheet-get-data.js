...
function pickRangeValue(r, idx, rw, displayRows) {
  ...
  if (disp !== undefined && disp !== null && disp !== "") {
    const cleaned = sanitizeRangeCell(String(disp).trim());
...
function pickPlatformValue(r, idx, rw, displayRows) {
  const disp = pickDisplay(r, idx, rw);
  if (disp && !isDateLikeLabel(disp)) return disp;
  const raw = pick(rw, idx);
...
function parseToolsFromSheet(rows, displayRows, categoryId, startToolId) {
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    let openDate = "";
    if (openDateCol >= 0) {
      openDate = pickDisplay(r, openDateCol, rw) || pick(rw, openDateCol);
    }
