...
function isEthereumLiquidityStatusCellEmptyRaw(rawVal, displayVal) {
  const disp =
    displayVal != null && String(displayVal).trim() !== "" ? String(displayVal).trim() : "";
  if (disp) return false;
  if (rawVal === false || rawVal === 0) return false;
  if (rawVal === true || rawVal === 1) return false;
...
function isEthereumLiquidityStatusVisible(rawVal, displayVal, link) {
  if (isEthereumLiquidityStatusOpen(rawVal, displayVal)) return true;
  if (!isEthereumLiquidityStatusCellEmptyRaw(rawVal, displayVal)) return false;
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    if (categoryId === "ethereum") {
      ...
      if (
        ...
        !isEthereumLiquidityStatusVisible(
