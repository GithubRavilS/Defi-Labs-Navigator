/**
 * Проверка пароля VIP и сессии для DeFi Labs Navigator (Vercel + MySQL).
...
function signNavToken({ clientId, codeVersion, validUntil }, jwtSecret) {
  ...
  return jwt.sign(
    { sub: clientId, v: codeVersion, typ: 'nav' },
    jwtSecret,
    { algorithm: 'HS256', expiresIn: maxSec }
  );
}
